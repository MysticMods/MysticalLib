...
public class BakedModelFence extends BakedModelBlock {
  ...
  private void getFaceQuads(@Nonnull List<BakedQuad> quads, @Nullable Direction side, @Nullable BlockState state) {
    if (state == null) {
      ...
      boolean cnorth = state.get(FenceBlock.NORTH);
      boolean csouth = state.get(FenceBlock.SOUTH);
      boolean cwest = state.get(FenceBlock.WEST);
      boolean ceast = state.get(FenceBlock.EAST);
