...
public class BakedModelWall extends BakedModelBlock {
  ...
  private void getFaceQuads(@Nonnull List<BakedQuad> quads, @Nullable Direction side, @Nullable BlockState state) {
    if (state == null) {
      ...
      boolean cnorth = state.get(WallBlock.NORTH);
      boolean csouth = state.get(WallBlock.SOUTH);
      boolean cwest = state.get(WallBlock.WEST);
      boolean ceast = state.get(WallBlock.EAST);
      if (up) {
        post.addToList(quads, side);
      }
      if (cnorth) {
