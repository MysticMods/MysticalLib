...
/**
 * Mystical lib implementation of a FluidTank for storage.
 */
public class SmartTank extends FluidTank {
  ...
  public boolean canDrain(@Nullable Fluid fl) {
    ...
    return SmartTank.areFluidsTheSame(fl, fluid2.getFluid());
  }
  ...
  /**
   * Sets if the tank can be drained
   */
  ...
  public boolean canFill(@Nullable Fluid fl) {
    if (fl == null || !canFillFluidType(new FluidStack(fl, 1))) {
      ...
      return SmartTank.areFluidsTheSame(fluid.getFluid(), fl);
    } else {
      return true;
    }
  }
